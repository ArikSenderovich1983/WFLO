...
\textcolor{purple}{In this work, we use the Fujitsu Digital Annealer (DA), a recent computer architecture designed for solving QUBO problems \cite{matsubara2020digital}. The third generation DA (DA3), a hybrid system of hardware and software, is capable of representing QUBOs with up to 100000 variables. For our DA environment, the coefficients for the quadratic terms range from $-2^{62}$ to $2^{62}$ and those for the linear terms range from $-2^{73}$ to $2^{73}$ \cite{DA3}. The algorithm used by the DA is based on simulated annealing \cite{kirkpatrick1983optimization}, however it takes advantage of the massive parallelization provided by the custom CMOS hardware and uses a dynamic offset mechanism to escape local minima \cite{aramon2019physics}. In addition, DA supports a dedicated bit flip mechanism, over a subset of variables belonging to one-hot equivalent constraints when using DA3. The Fujitsu Digital Annealer has been previously applied to variety of problems in different research areas such as machine learning \cite{cohen2020ising,salehinejad2019ising} and communication \cite{naghsh2019digitally,rahman2019ising}.}
...
Our main results are: \begin{itemize}
	\item For given time limit, the Digital Annealer (DA) achieves a solution quality as good or as better than existing state-of-the-art exact methods on \textcolor{purple}{8 of 12 instances}.
	\item To achieve such a solution quality, the runtime for solving \quls{} on the DA is at least \textcolor{purple}{one orders of magnitude} shorter than \qcls{} on state-of-the-art commercial solvers for hard WFLO instances.  
	\item Given sufficient runtime, solving the \qcls{} model with commercial software yields strong results on most of the instances. Compared to previous exact models (such as \qcss), \qcls{} is able to find equal or better solutions on \textcolor{purple}{6 of 12 benchmark problems and for 3 of the 6 hard ones.} %  \todo{Arik: Is this correct? Table 2 shows DA@10seconds with the best solution found for 7/12 instances and \qcls{}grb@3600 for 6/12. And 4/6 vs. 3/6 for the WR36). Are you comparing \qcls{}grb with \qcssgrb and ILP-LS? We seem to be underplaying DA performance in this comment. Chris: I changed to competitive results. You're right, I'd say that the DA is not `losing' to \qcls{}grb.}  
\end{itemize}
...
We used state-of-the-art commercial software, \textcolor{purple}{Gurobi v9.1.1~\cite{gurobi}}, to solve \qcls{}, \ilpls{} and \qcss. 
...
found until that point in time. The exact methods were solved using Gurobi's integer programming and quadratic programming solvers on a \textcolor{purple}{Window PC with Intel(R) Core(TM) i7-8700K CPU @3.20GHz with 16 GB RAM.}

For the DA, we used a single configuration that was found to work best across the different instances. 
The DA was run for increasingly large time limits \textcolor{purple}{ranging from $1$ second to $10$ seconds} where we no longer observe an improvement in 
performance. The penalty terms $\lambda_1$ and $\lambda_2$ (see~Eq.~\eqref{QUBO}) are selected such that $\lambda_1= \lambda_2$ \textcolor{purple}{and their magnitudes are tuned by DA3 automatically.}
...
the three quadratic LS-based approaches (\qulsda{}, \qulsgrb{}, and \qclsgrb{}) dominate the others by finding the best (or equal) solutions in \textcolor{purple}{all 6 instances. For easier instances, 
the DA-based approach (\qulsda{}) is also dominating as it finds the best solutions in 5 out of 6 solutions.}
...
while \qclsgrb{} finds the best solutions in \textcolor{purple}{6} out of the 12 scenarios. 
...
We now turn to present the mean relative error over time per approach. Note that lower values indicate better performance.  \textcolor{purple}{Figure~\ref{fig:results1} 
...
\textcolor{purple}{Specifically, for most instances, \qclsgrb{} gives the best solutions at 3600 seconds, while the DA-based approach \qulsda{}
yields competitive solutions after less than 10 seconds. However, the good performance of \quls{} only occurs when the solver is the DA. In fact, \qulsgrb{} trails all the other methods.
Furthermore, 
...
\textcolor{purple}{The results also show that on average 
the \ilplsgrb{} method starts with poor solutions, and ends up improving over \qcssgrb{} after around 3 seconds.}
...
The average speed-up factors were found to be \textcolor{purple}{$666$ and $100$ for $1$ and $10$ seconds, respectively.} \textcolor{red}{(Arik, please change the speed-up ratios, the rest of the paragraph, and next paragraph.)}
...
\begin{figure}[t]%
	...
	\includegraphics[width=\textwidth]{Figures/MRE_100s_DA3.png}  
	\caption{Mean Relative Energy (wrt best feasible solution) vs. Runtime (after 100 seconds).}%
...
\textcolor{purple}{Our results show numerically that the quadratic programming models that we
...
only to the \qclsgrb{} and outperforms all methods in terms of performance in short runtimes} (a speed-up factor of 3 orders of magnitude when the DA runs for $1$ second) \textcolor{red}{(Arik, please change the speed-up factors.)}.
