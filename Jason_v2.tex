...
Our main results are: \begin{itemize}
	...
	\item This performance is achieved for the \quls{} model with a runtime that is at least one order of magnitude shorter than \qcls{} on state-of-the-art commercial solvers for hard WFLO instances. \todo{Jason: This claim (``one order of magnitude'') need to be made consistent with the claims and results later in the papaer.}
...
yields the best solutions after 10 seconds. However, the good performance of \quls{} only occurs when the solver is the DA. In fact, \qulsgrb{} trails all the other methods.
Furthermore, 
...
The average speed-up factors were found to be $666$ and $100$ for $1$ and $10$ seconds, respectively. \todo{(Arik, please change the speed-up ratios, the rest of the paragraph, and next paragraph.)}
...
only to the \qclsgrb{} and outperforms all methods in terms of performance in short runtimes (a speed-up factor of 3 orders of magnitude when the DA runs for $1$ second) \todo{(Arik, please change the speed-up factors.)}.
...
\todo{Arik \& Jason: do these need to be updated?}
