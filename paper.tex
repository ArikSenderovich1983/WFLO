...
Our main results are: \begin{itemize}
	\item WFL-QUBO when solved using the digital annealer (DA) achieves a similar performance to state-of-the-art exact methods.
		\item The runtime for solving WFL-QUBO on the DA is at least two orders of magnitude lower than WFL-QCOP for hard WFLO instances.  
	\item Given sufficient runtime, solving the WFL-QCOP model yields superior results on most of the instances.
\end{itemize}
...
For the $36$ wind directions setting, the velocities came from the probability distribution presented in Figure~\ref{fig:prob_wind}.
...
We used state-of-the-art commercial software, Gurobi~\cite{gurobi}, to solve WFL-QCOP, ILP-SS and QP-SS. 
...
performance. The penalty terms $\lambda_1$ and $\lambda_2$ (see~Eq.~(\ref{QUBO}) are selected such that $\lambda_1= \lambda_2$ and sum of the two penalty terms, namely $\lambda_1+\lambda_2$, 
...
Then, for every point in time $t \in \{0,\ldots, 3600\}$ (measured in seconds), we can compute the relative error of a given approach per instance $w$. Let $\mathcal{A}$ be the set of solution approaches, $\mathcal{A}= \{QUBO-LS, QP-LS, DA-LS, ILP-LS, QP-SS\}$. Further, denote by $b_{w,t,a}$ the best solution attained before time $t$ of approach $a$ for instance $w$. Then, the mean relative error at time $t$ for approach $a$ on instance $w$ is given by \begin{equation}MRE(w,t,a) = \frac{b_w - b_{w,t,a}}{b_w}.\end{equation} Note that the expression is always positive, since we are solving a maximization problem. We can now average over the different instances at time $t$. The average performance of approach $a$ at time $t$, $MRE(t,a)$ can be computed as, \begin{equation} MRE(t,a) = \frac{1}{|\mathcal{W}|}\sum_{ w\in\mathcal{W}} MRE(w,t,a).\end{equation}
We present $MRE(t,a)$ for each of the approaches to demonstrate the speed of convergence of the proposed LS-based methods.

\todo{@Chris - new part:} The last measure that we introduce is the speedup factor $S(a_1, a_2, t, w)$. The measure compares the time that it takes
...
with $E_{SS}(a, t, w)$ being the SS energy value under approach $a$ for instance $w$ by time $t$. The speedup factor 
is useful to measure how many times faster (or slower) approach $a_1$ solves problem $w$ compared to approach $a_2$.   
For example, $$S(QP-LS, DA-LS, 10sec, w_1)$$ is computed as the ratio between the time that it takes QP-LS to 
reach the same quality of solution as DA-LS achieves after $10$ seconds (over problem $w_1$). Since
we limit the runtime of the exact methods by $3600$ seconds, we may get a lower bound on the speedup factor. 
Therefore, if for the example above the result is $3600/10$, we shall interpret it as a speedup factor of \emph{at least} $360$. \todo{@Chris - new part ends here.}
...
\todo{@Chris - new part:} To demonstrate the gain in runtime from using the DA, we compare the average speedup factor between
the QP-LS (the dominant exact method) to a DA-LS solutions after $1$ and $10$ seconds.
In other words, $w \in \mathcal{W}$ we computed $S(QP-LS, DA-LS, 1sec, w)$ and $S(QP-LS, DA-LS, 10sec, w)$
for every $w$ 
and averaged the speedup factors across the instances. 

As a result, the average speedup factors were found to be $666$ and $100$ for $1$ and $10$ seconds, respectively.\footnote{The speedup factors  were rounded to have 0 values after the floating point.} These results show that the DA achieves competitive solutions after running for $1$ second.
Note that since for many of the instances QP-LS did not attain
the energy level of DA-LS at time $t$ (1 or 10 seconds), we consider the speedup factor to be a lower bound, meaning
that the DA-LS gives potentially an even higher speedup factor, which we did not compute due to the time limit of $3600$. \todo{@Chris - new part ends here}
...
\todo{@Chris - please check discussion:}
...
First, the two problems are the basis for the QP-LS and QUBO-LS methods that solved with Gurobi. The two methods, namely QP-LS and QUBO-LS, turn out to be powerful approaches to solving WFLO. Specifically, QP-LS yields the best performance (after 3600 seconds) for most problem instances
and especially,
...
instances, shows competitive performance on the other instances, and outperforms all methods in terms of runtime (speedup factor of 2 orders of magnitude when the DA runs for $1$ second).
...
models (WFL-QUBO). The former can be used to solve WFLO using advanced optimization software,
...
that the computational complexity of WFLO is $\mathcal{NP}$-hard. However,
