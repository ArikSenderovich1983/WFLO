...
Our main results are: \begin{itemize}
	\item WFL-QUBO when solved using the digital annealer (DA) achieves a solution quality as good or better than the state-of-the-art exact methods on 10 of 12 instances.
		\item To achieve such a solution quality, the runtime for solving WFL-QUBO on the DA is at least two orders of magnitude lower than WFL-QCOP for hard WFLO instances.  
	\item Given sufficient runtime, solving the WFL-QCOP model yields competitive results on most of the instances. \todo{Arik: Is this correct? Table 2 shows DA@10seconds with the best solution found for 7/12 instances and QP-LS@3600 for 6/12. And 4/6 vs. 3/6 for the WR36). Are you comparing QP-LS with QP-SS and ILP-LS? We seem to be underplaying DA performance in this comment. Chris: I changed to competitive results. You're right, I'd say that the DA is not `losing' to QP-LS.}  
\end{itemize}
...
We compared our two models, WFL-QCOP and WFL-QUBO, against two state-of-the-art exact optimization methods:
the integer linear programming (ILP-LS) approach~\cite{donovan2005wind,Zhang2014} and the 
model that uses a quadratic approximation (QP-SS)~\cite{turner2014new}. 
The ILP model is a standard, exact linearization of the WFL-QCOP model.
...
However, the ILP is solved using linear optimization approaches, while WFL-QCOP is solved using quadratic programming methods as mentioned in Section~\ref{quadratic}. Hence
...
We used state-of-the-art commercial software, Gurobi~\cite{gurobi}, to solve WFL-QCOP, ILP-LS and QP-SS. 
For consistency, we shall refer to WFL-QCOP problem as QP-LS as it is a quadratic program that uses the LS objective.
In addition, we 
used both Fujitsu's Digital Annealer and Gurobi to solve WFL-QUBO: we shall refer to the former as DA-LS, and to the latter
as QUBO-LS. \todo{Arik: Is it necessary to change the names WFL-QUBO -> QUBO-LS, WFL-QCOP -> QP-LS? This makes it confusing for someone who skims over this paragraph.Can't we use the same names (one or the other) through-out? Chris: The issue is that WFL-QUBO and WFL-QCOP are problem names without mentioning of the objective - I need to differentiate QP-SS from QP-LS. Should I change everything backwards to the XX-LS notation? I could do that.}
...
found until that point in time. The exact methods were all solved using Gurobi's quadratic programming solver on a 
Ubuntu PC with Inte(R) Core(TM) i7-9700 8 core CPU @3.00 GHz with 12 MB cache and 8 GB RAM.
...
is set to the highest value that the Digital Annealer allows.\footnote{The DA has an upper limit on the size of objective function coefficients.} 
...
The only method that directly optimizes SS is QP-SS. Hence, for the other methods, we converted solutions
LS-based solution into the SS expression using Eq.~(\ref{eq:ss}) as proposed in Zhang et al.\ \cite{Zhang2014}. Specifically, let $x = (x_1, \ldots, x_k), k=n^2$ be a turbine placement solution obtained by an LS-based method.
...
%For example, the best solution (within 3600 seconds) for the second instance $w_2$ that aims at placing $30$ turbines on a $10\times10$ grid with a single wind regime is attained by QUBO-LS and the SS energy expression is $15742$ (kW); thus, we set $b_w = 15742$.
Then, for every point in time $t \in \{0,\ldots, 3600\}$ (measured in seconds), we can compute the relative error of a given approach per instance $w$. Let $\mathcal{A}$ be the set of solution approaches, $\mathcal{A}= \mbox{
	\{\emph{QUBO-LS}}, \mbox{\emph{QP-LS}}, \mbox{\emph{DA-LS}}, \mbox{\emph{ILP-LS}}, \mbox{\emph{QP-SS}}\}$. Further, denote by $b_{w,t,a}$ the best solution attained before time $t$ of approach $a$ for instance $w$. Then, the relative error at time $t$ for approach $a$ on instance $w$ is given by
\begin{equation}
  RE(w,t,a) = \frac{b_w - b_{w,t,a}}{b_w}.
\end{equation} Note that the expression is always positive, since we are solving a maximization problem. We can now average over the different instances at time $t$. The average performance of approach $a$ at time $t$, $MRE(t,a)$ can be computed as, \begin{equation} MRE(t,a) = \frac{1}{|\mathcal{W}|}\sum_{ w\in\mathcal{W}} RE(w,t,a).\end{equation}
...
Formally, $$S(a_1, a_2, t, w) = \frac{\min \big[\{ t' : E_{SS}(a_1, t',w) = E_{SS}(a_2, t, w) \} \cup \{3600\} \big] }{t}, $$
...
For example, $$S(\mbox{\emph{QP-LS}}, \mbox{\emph{DA-LS}}, 10, w_1)$$ is the ratio between the time that it takes QP-LS to 
reach the same quality of solution as DA-LS achieves after $10$ seconds on problem $w_1$. Since
...
\begin{table}[t!]
	...
	\vspace{0.5em}
	\caption{Total Expected Power (kW) after 100 seconds for the exact techniques and by 10 seconds for DA-LS.}\label{tab:results1}
\end{table}
...
\begin{table}[t!]
	...
	\vspace{0.5em}
	\caption{Total Expected Power (kW) after 3600 seconds and by 10 seconds for DA-LS.}\label{tab:results2}
\end{table}
...
presents the MRE after $50$ seconds, while Figure~\ref{fig:results2} corresponds
...
Our proposed quadratic models, QP-LS and DA-LS,
outperform the rest of the techniques both in terms of runtime (best results in short time), and in terms of overall performance.
...
Furthermore, 
QP-LS converges to the best solution faster than all other exact methods. In fact, it is inferior only to the approximate DA-LS method.
...
To demonstrate the gain in runtime from using the DA, we compare the average speed-up factor between
the QP-LS (the dominant exact method) to a DA-LS solutions after $1$ and $10$ seconds.
For each $w \in \mathcal{W}$ we computed the mean $S(\mbox{\emph{QP-LS}}, \mbox{\emph{DA-LS}}, t, w)$ for $t \in \{1,10\}$. 
%and $S(QP-LS, DA-LS, 10sec, w)$
%for every $w$ 
...
Per instance, the minimum speedup was $1$ for $1$ second and $0.1$ for $10$ seconds, meaning that for some of the instances DA-LS was actually
slower than QP-LS. The maximum speedup was (at least) $3600$ for $1$ second and (at least) $360$ for $10$ seconds: for those instances
QP-LS was not able to find a comparable solution to DA-LS within the $3600$ time limit. \todo{Arik: What about a table -- perhaps in an Appendix? If not, you should at least quote the min and max speed-up factors and comment on the speed-up comparing DA with the other techniques. Chris: I added min/max and comparison to the two other LS-based methods.}
...
Note that since for many of the instances QP-LS did not attain
the energy level of DA-LS at time $t$ (1 or 10 seconds),
%we consider the speed-up factor to be a lower bound, meaning
that
the DA-LS speed-up is actually even higher. When compared
to the other LS-based methods, the DA speeds up by a higher factor compared to its speedup w.r.t QP-LS. At $1$ ($10$) seconds, the speedup factor compared to ILP-LS 
is $1385$ ($180$) and for the QUBO-LS it is $900$ ($152$). Here, we compare only LS based methods as they optimize on the same objective as the DA.  
...
\todo{Arik: Should this be Sectiobn 5 rather than 4.3? Chris: I am fine with both. I felt that it was strongly tied to 4.2, but I can see it becoming a short standalone section.}
...
First, the two problems are the basis for the QP-LS and QUBO-LS methods solved with Gurobi. The two methods are powerful approaches to solving WFLO. Specifically, QP-LS yields the best performance (after 3600 seconds) for most problem instances
and especially,
for the hard instances with 36 wind directions. In addition, the runtime performance of QP-LS is second
only to the Digital Annealer.  Second, the QUBO model can be solved with recent specialized hardware, Fujitsu's DA, as demonstrated by the DA-LS approach. DA-LS yields the best results in 7 out of 12 
instances, shows competitive performance on the other instances, and outperforms all methods in terms of runtime (a speed-up factor of 3 orders of magnitude when the DA runs for $1$ second).


The ability of DA-LS to quickly obtain competitive solutions on large WFLO instances 
is crucial when solving
multiple wind farm layout problems. Moreover, it allows the flexibility to tune the various WFLO parameters (including turbine specs, number of turbines, and farm sizes) without incurring a steep computational cost that come
...
The framework is embodied in two models: constrained (WFL-QCOP) and unconstrained binary
 (WFL-QUBO). The former can be used to solve WFLO using advanced optimization software,
