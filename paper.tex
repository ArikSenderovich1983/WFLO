...
WFLO/paper.tex
@ArikSenderovich1983 ArikSenderovich1983 fixes in 3, pass over 4 and 5.
8cbc7fe 8 hours ago
@ArikSenderovich1983@jcb-uoft@JasonZhangjc
908 lines (744 sloc)  54.1 KB
...
than software-based approaches. The ability to quickly obtain competitive solutions on large WFLO instances 
can be employed to solve
multiple wind farm design problems. 
...
naturally results in a quadratic objective function in our WFLO formulations. Such functions have been the subject of recent research advances~\cite{ku2017hybrid,bian2010ising}, motivating the contributions here.
...
Another non-linear optimization approach was proposed in Ulku and Alabas-Uslu\ \cite{ulku2019new}.
Ulku and Alabas-Uslu\ \cite{ulku2019new}, replaced integer placement variables with continuous variables between $0$ and $1$,
and approximated the constraints such that the resulting variables yield an integer placement.
Their approach provides yet another approximation to the WFLO problem considered in~\cite{turner2014new}.
...
%proposed in
Unlike Turner et al.\ \cite{turner2014new} nor Ulku and Alabas-Uslu\ \cite{ulku2019new}, our model is not an approximation of the SS objective, 
...
These results lead to the conclusion that both types of methods (exact and approximate) should be considered and, therefore, we introduce a unified paradigm, namely quadratic programming, that enables
...
penalizing solutions that violate the exactly $m$ turbines constrain and the term $\lambda_2 \sum_{(i,j) \in \mathcal{E}}^{} x_i x_j$ penalizing
...
WFLO is a type of strictly convex integer quadratically constrained optimization problem (IQCOP), which is known to be NP-hard \cite{van1981another}. The common exact methods for solving
IQCOPs are tree search \cite{land2010automatic} type algorithms with a variety of techniques to prune sub-trees that definitely do not contain the optimal solution.

In operations research, the generic exact approach for solving IQCOPs are the branch-and-cut based mixed integer programming (MIP) solvers \cite{bonami2008algorithmic}. By solving a relaxed problem to bound the objective value at each node and strengthening the relaxed problem with cutting planes to restrict the feasible region further, MIP solvers has been proved effective for attacking IQCOPs \cite{junger200950}.
...
\todo{@Arik and Chris: please correct Jason's writing errors}
...
\begin{table}[t!]
	...
	\vspace{0.5em}
	\caption{Total Expected Power (kW) after 100 seconds as function of benchmark and method. \todo{@Arik: could you please why we do not mention any optimality here?} }\label{tab:results1}
\end{table}
