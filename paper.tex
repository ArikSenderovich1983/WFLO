...
Since a special case of \qcls{} is  $\mathcal{NP}$-hard it implies that \qcls{} is at least $\mathcal{NP}$-hard. On the other hand,
...
Our main results are: \begin{itemize}
	\item \quls{} when solved using the digital annealer (DA) achieves a solution quality as good or better than the state-of-the-art exact methods on 10 of 12 instances.
		\item To achieve such a solution quality, the runtime for solving \quls{} on the DA is at least two orders of magnitude lower than \qcls{} for hard WFLO instances.  
	\item Given sufficient runtime, solving the \qcls{} model yields strong results on most of the instances. Compared to previous exact models (such as \qcss), \qcls{} is able to find equal or better solutions on 8 of 12 benchmark problems and for 5 of the 6 hard scenarios. %  \todo{Arik: Is this correct? Table 2 shows DA@10seconds with the best solution found for 7/12 instances and \qcls{}grb@3600 for 6/12. And 4/6 vs. 3/6 for the WR36). Are you comparing \qcls{}grb with \qcssgrb and ILP-LS? We seem to be underplaying DA performance in this comment. Chris: I changed to competitive results. You're right, I'd say that the DA is not `losing' to \qcls{}grb.}  
\end{itemize}
...
We shall denote the problems using their names and the solvers used as follows: \qclsgrb{} (GRB stands for Gurobi), \ilplsgrb{}, and \qcssgrb{}.
In addition, we 
...
The only method that directly optimizes SS is \qcssgrb{}. Hence, for the other methods, we converted solutions
...
Then, for every point in time $t \in \{0,\ldots, 3600\}$ (measured in seconds), we can compute the relative error of a given approach per instance $w$. Let $\mathcal{A}$ be the set of solution approaches, $$\mathcal{A}= \mbox{
	\{\emph{\qulsgrb}}, \mbox{\emph{\qclsgrb}}, \mbox{\emph{\qulsda}}, \mbox{\emph{\ilplsgrb}}, \mbox{\emph{\qcssgrb}}\}.$$ Further, denote by $b_{w,t,a}$ the best solution attained before time $t$ of approach $a$ for instance $w$. Then, the relative error at time $t$ for approach $a$ on instance $w$ is given by
\begin{equation}
...
the three quadratic LS-based approaches (\qulsda{}, \qulsgrb{}, and \qclsgrb{}) dominate the others by finding the best (or equal) solutions in 11 out of 12 instances. For easier instances, 
...
Per instance, the minimum speedup was $1$ for $1$ second and $0.1$ for $10$ seconds: for some instances \qclsgrb{} achieved the same quality as \qulsda{} in less time. The maximum speedup was (at least) $3600$ for $1$ second and (at least) $360$ for $10$ seconds: for those instances
\qclsgrb{} was not able to find a comparable solution to \qulsda{} within the $3600$ time limit. As a consequence, 
the \qulsda{} speed-up is actually even higher than computed above. \end{sloppypar}
...
When compared
to the other LS-based methods, the DA speed-up is even larger than when compared to \qclsgrb{}.  At $1$ ($10$) seconds, the speedup factor compared to \ilplsgrb{} 
...
For full speedup results see Tables~\ref{tab:results3} and~\ref{tab:results4} in the Section~\ref{app1}.
...
The framework is embodied in two models: constrained (\qcls) and unconstrained binary
 (\quls). The former can be used to solve WFLO using advanced optimization software,
...
for every LS-based method (computed using the SS energy term). The speedup is calculated according to Eq.~(\ref{eq:speedup}). Table~\ref{tab:results4} show similar results for the speedup factor
compared to DA at 10 seconds. Note that N/A values
indicate that the solver has gotten an LS solution within 3600 seconds that turned out to be worse
than a DA solution after 1 (or 10 seconds). The average lower bound on the speedup at $1$ seconds
is 665,	1511, and 949 for \qclsgrb, \qulsgrb, and \ilplsgrb, respectively. For the $10$ seconds
results, the average
speedup factor is at least 99, 196, and	166 for the respective methods.
For instances where speedup factor in Table~\ref{tab:results4} is 10 times less than in Table~\ref{tab:results3},
the DA has found the 10 seconds solution after 1 second.
