...
Note that here we refer to $i$ as a potential turbine location and not the $i$th turbine as we did in Section~\ref{sec:related}.\todo{JCB: This is an unnecessary change. Go back and reformulate Section~\ref{sec:related} to refer to the potential position $i$ rather than turbine $i$ so that we do not have this inconsistency.}
...
\todo{JCB: We need to formally present the ILP model in the experiment section. We can then discuss the fact that the model is identical - because the linearization gives and exact representation.}
