...
across the different LS-based methods. For example, the best solution (within 3600 seconds) for the second instance $w_2$ that aims at placing $30$ turbines on a $10\times10$ grid with a single wind regime is attained by QUBO-LS and the SS energy expression is $15742$ (kW); thus, we set $b_w = 15742$. Then, for every point in time $t \in \{0,\ldots, 3600\}$ (measured in seconds), we can compute the relative error of a given approach per instance $w$. Let $\mathcal{A}$ be the set of solution approaches, $\mathcal{A}= \{QUBO-LS, QP-LS, DA-LS, ILP-LS\}$. Further, denote by $b_{w,t,a}$ denote the best solution attained before time $t$ of approach $a$ for instance $w$. Then, the mean relative error at time $t$ for approach $a$ on instance $w$ is given by \begin{equation}MRE(w,t,a) = \frac{b_{w,t,a}}{b_w}.\end{equation} We can now average over the different instances at time $t$. The average performance of approach $a$ at time $t$, $aMRE(t,a)$ can be computed as, \begin{equation}aMRE(t,a) = \frac{1}{|\mathcal{W}|}\sum_{ w\in\mathcal{W}} MRE(w,t,a).\end{equation}
We shall present $aMRE(t,a)$ for each of the approaches to demonstrate the speed of convergence of the proposed LS-based methods. \todo{Jason: please check that this is consistent with how you calculated MRE in the experiment.} 
...
\begin{table}[t!]
	...
	\vspace{0.5em}
	\caption{Total Expected Power (kW) after 100 seconds as function of benchmark and method. * stands for optimal solution. \todo{@Jason: could you please check if my marking of * for optimal solutions is correct? I think that optimality was proven only for $m=20$ and single wind direction, right?} }\label{tab:results1}
\end{table}
...
\begin{table}[t!]
	...
	\vspace{0.5em}
	\caption{Total Expected Power (kW) after 3600 seconds as function of benchmark and method. * stands for optimal solution.}\label{tab:results2}
\end{table}
...
We now turn to present the $aMRE(a,t)$ over time: lower values indicate better performance. The different lines 
correspond to the 5 approaches. The horizontal axis represents time in seconds. Figure~\ref{fig:results1} 
presents the first $50$ seconds focusing on speed of performance, while Figure~\ref{fig:results2} corresponds
to the full run ($3600$ seconds).   

\begin{figure}[t]%
	
	\includegraphics[width=\textwidth]{energy_50.png}  
	\caption{Mean Relative Energy (w.r.t. best feasible solution) vs. Runtime (first 50 seconds).}%
...
\begin{figure}[t]%
	...
	\caption{Mean Relative Energy (w.r.t. best feasible solution) vs. Runtime (3600 seconds).}%
