...
To demonstrate the speedup gained from using the DA, we compare the point in time when the DA reaches 
a competitive solution (10 seconds) to the point in time when QP-LS reaches the same solution in terms of energy.
Table~\ref{speedup} summarizes the performance of QP-LS when compared to that of the DA after 10 seconds.
We see a speedup factor of X. \todo{discuss with Chris}
...
instances, shows competitive performance on the other instances, and outperforms all methods in terms of runtime (\todo{speed-up ratio of ...})
