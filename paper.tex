...
Our main results are: \begin{itemize}
	...
	\item Given sufficient runtime, solving the WFL-QCOP model yields strong results on most of the instances. Compared to previous exact models (QP-SS and ILP-LS), WFL-QCOP is able to find equal or better solutions on 8 of 12 benchmark problems and for 5 of the 6 hard scenarios. %  \todo{Arik: Is this correct? Table 2 shows DA@10seconds with the best solution found for 7/12 instances and QP-LS@3600 for 6/12. And 4/6 vs. 3/6 for the WR36). Are you comparing QP-LS with QP-SS and ILP-LS? We seem to be underplaying DA performance in this comment. Chris: I changed to competitive results. You're right, I'd say that the DA is not `losing' to QP-LS.}  
\end{itemize}
...
as QUBO-LS. \todo{Arik: I think it is important to use consistent names through-out. The problem is that we do not have a unified naming format. The problems can be QC-LS, QU-LS (for Quadratic Constraint/Unconstrained Linear Superposition) instead of WFL-QCOP QFL-QUBO. Then the results need to include the solver and so we have QU-LS(DA), QU-LS(GRB), QC-LS(GRB), QC-SS(GRB), and LC-LS(GRB). (LC = Linear Constrained). I suggest you use Latex macros so that we can easily change then all again if necessary.}
...
To demonstrate the gain in runtime from using the DA, we compare the average speed-up factor between
 QP-LS and a DA-LS solutions after $1$ and $10$ seconds.
...
Per instance, the minimum speedup was $1$ for $1$ second and $0.1$ for $10$ seconds: for some instances QP-LS achieved the same quality as DA-LS in less time. The maximum speedup was (at least) $3600$ for $1$ second and (at least) $360$ for $10$ seconds: for those instances
QP-LS was not able to find a comparable solution to DA-LS within the $3600$ time limit. As a consequence, 
the DA-LS speed-up is actually even higher than computed above. \todo{Arik: I think a table of speed-ups per problem instance is still necessary in an Appendix.}
...
%Many of the instances running QP-LS for 3600 seconds did not result in as good a solution as DA-LS at time $t$ (1 or 10 seconds).
%we consider the speed-up factor to be a lower bound, meaning
When compared
to the other LS-based methods, the DA speed-up is even larger than when compared to QP-LS.  At $1$ ($10$) seconds, the speedup factor compared to ILP-LS 
is $1385$ ($180$) and for the QUBO-LS it is $900$ ($152$). We compare against the LS based methods as they directly optimize on the same objective as the DA.  
