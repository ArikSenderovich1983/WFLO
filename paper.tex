...
is best captured by Jensen's wake model~\cite{jensen1983note} when superimposed
...
An alternative formulation of WFLO relies on a less accurate wakes model, which uses linear superpositioning
...
that approximate wake dependencies, since until recent years solving linear programming problems
...
problem (QCOP).  The QCOP can be solved quickly using state-of-the-art solvers
...
The remainder is structured as follows. Section~\ref{sec:related} presents our wind farm model and a literature review of exact and approximate
optimization solutions to the layout problem. The main contribution, namely the two quadratic models, is introduced in Section~\ref{sec:QUBO4WFLO}.
Section~\ref{sec:eval} demonstrates the usefulness of our approach via a thorough empirical evaluation of the proposed methods. 
...
and 3 variations for number of turbines ($m \in \{20, 30 or 40\}$). We denote by $\mathcal{W}$ the set of 12 instances. 
The wind regime distribution presented in Figure~\ref{fig:prob_wind} was 
elicited from~\cite{Zhang2014}.
...
We have measured the total expected energy of a solution using the SS expression (see Eq.~(\ref{eq:ss}))\footnote{Note that the total mean energy is presented in $kW/year$.}.
...
compared the average performance of all LS-based methods on all 12 WFLO instances \footnote{The QP-SS method was not considered for performance over time, since it optimizes a different objective function.}. Since energy levels are not directly comparable between the different instances (due to varying number of turbines and wind farm specs) we used the \emph{mean relative error} (MRE) measure.

The MRE is defined with respect to the best solution obtained per instance and can be computed over time. Then, the
time-varying MRE can be averaged across the different scenarios and the average MRE (aMRE) can be used
for the desired comparison. We shall demonstrate the construction below.
...
across the different LS-based methods. For example, the best solution (within 3600 seconds) for the second instance $w_2$ that aims at placing $30$ turbines on a $10\times10$ grid with a single wind regime is attained by QUBO-LS and the SS energy expression is $15742 kW/year$. Therefore, we let $b_w = 15742$. Then, for every point in time $t \in \{0,\ldots, 3600\}$ (measured in seconds), we can compute the relative error of a given approach per instance $w$. Let $\mathcal{A}$ be the set of solution approaches, $\mathcal{A}= \{QUBO-LS, QP-LS, DA-LS, ILP-LS\}$. Further, denote by $b_{w,t,a}$ denote the best solution by time $t$ of approach $a$ over instance $w$. Then, the mean relative error at time $t$ for approach $a$ on instance $w$ is given by $$MRE(w,t,a) = \frac{b_{w,t,a}}{b_w}.$$ We can now average over the different instances at time $t$. The average performance of approach $a$ at time $t$, $MRE(t,a)$ can be computed as $$MRE(t,a) = \frac{1}{|\mathcal{W}|}\sum_{ w\in\mathcal{W}} MRE(w,t,a).$$
We shall present this quantity ($MRE(t,a)$) for each of the approaches over time to demonstrate the speed of convergence of the proposed LS-based methods.  
...
Table~\ref{tab:results1} contains the best energy (in kW/hour) attained by the methods
for the 12 WFLO instances after $100$ seconds of runtime for the exact methods. The runtime for DA-LS was always less than $10$ seconds.
We observe that...

Table~\ref{tab:results2} shows the same type of results after $3600$ seconds. 
...
\begin{table}[t]
	...
	\vspace{0.5em}
	\caption{Total Expected Power (kW/year) after 100 seconds as function of benchmark and method. * stands for optimal solution. \todo{@Jason: could you please check if my marking of * for optimal solutions is correct? I think that optimality was proven only for $m=20$ and single wind direction, right?} }\label{tab:results1}
\end{table}
...
\vspace{0.5em}
\caption{Total Expected Power (kW/year) after 3600 seconds as function of benchmark and method. * stands for optimal solution.}\label{tab:results2}
\end{table}
...
Future work: additional wind farm settings (noise, terrain...). Convert into qubo.
