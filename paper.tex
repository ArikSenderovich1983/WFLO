...
as QUBO-LS. \todo{Arik: Is it necessary to change the names WFL-QUBO -> QUBO-LS, WFL-QCOP -> QP-LS? This makes it confusing for someone who skims over this paragraph.Can't we use the same names (one or the other) through-out?}
...
Then, for every point in time $t \in \{0,\ldots, 3600\}$ (measured in seconds), we can compute the relative error of a given approach per instance $w$. Let $\mathcal{A}$ be the set of solution approaches, $\mathcal{A}= \mbox{\emph{QUBO-LS}}, \mbox{\emph{QP-LS}}, \mbox{\emph{DA-LS}}, \mbox{\emph{ILP-LS}}, \mbox{\emph{QP-SS}}\}$. Further, denote by $b_{w,t,a}$ the best solution attained before time $t$ of approach $a$ for instance $w$. Then, the relative error at time $t$ for approach $a$ on instance $w$ is given by
\begin{equation}
...
The average speed-up factors were found to be $666$ and $100$ for $1$ and $10$ seconds, respectively. \todo{Arik: What about a table -- perhaps in an Appendix? If not, you should at least quote the min and max speed-up factors and comment on the speed-up comparing DA with the other techniques.}
...
that
the DA-LS speed-up is actually even higher.
...
\todo{Arik: Should this be Sectiobn 5 rather than 4.3?}
