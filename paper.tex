...
In this work, we use the first generation of the Fujitsu Digital Annealer (DA) \cite{aramon2019physics}, a specialized CMOS hardware capable of solving QUBO problems with up to 1024 binary variables. The algorithm used by the DA is based on simulated annealing \cite{kirkpatrick1983optimization}, however it takes advantage of the massive parallelization provided by the custom CMOS hardware and uses a dynamic offset mechanism to escape local minima \cite{aramon2019physics}. The Fujitsu Digital Annealer has been previously applied to variety of problems in different research areas such as machine learning \cite{cohen2020ising,salehinejad2019ising}, communication \cite{naghsh2019digitally,rahman2019ising}, etc.
	...
	standard exact optimization solvers that run until obtaining an optimal solution, the DA runs until reaching a time limit and returns the best found solution. {\color{purple} In our experiments, we analyze the quality of obtained solutions for different time limits.} \todo{EC: can be phrased instead as : In our experiments, we run the DA with increasingly large time limits until we observe no further improvement in objective. (see my point in Section 1.4)}
...
found until that point in time.
...
For the DA, we used a single configuration that was found to work best across the different instances. The initial temperature is 500000, the temperature mode is 0, the  interval for updating is 1, the offset increase rate is 10, the number of runs is 20, and the temperature decay rate is computed dynamically. \todo{Eldan and Arik: Do I need to write the formula for computing temperature decay rate? -- Jason} \todo{JCB: Yes, if it is not too complicated. You should mention if this is the standard thing that DA does or something you came up with.} The DA ran until convergence of the underlying simulated annealing algorithm and the runtime 
...
yields competitive solutions after less than 10 seconds. Furthermore, QP-LS converges to the best solution faster than all other exact methods. In fact, it is inferior only to the approximate DA-LS method.
...
First, the two problems are the basis for the QP-LS and QUBO-LS methods that solved with Gurobi. These methods turn out
to be a powerful approach to solving WFLO as it gives the best performance after 3600 seconds for most instances (7 out of 12), \todo{@Chris: I counted the number of instances for which the two methods show best results after 3600. is that ok?}
and especially,
...
instances, shows competitive performance on the other instances, and outperforms all methods in terms of runtime by two orders of magnitude.\todo{JCB: Is ``two orders of magnitude'' accurate? If you compare to 100, it is only 1 order of magnitude to achieve the ``same'' performance. Arik: I refer to 10 seconds for the DA, hence 2 orders.}
