...
Our main results are: \begin{itemize}
	\item QUBO based WFLO using the digital annealer is competitive with exact methods. 
	\item Given sufficient runtime the QCOP model provides superior results compared to state-of-the-art methods.
	\item The runtime for solving the QUBO model is two orders of magnitude lower than QCOP. 
\end{itemize}
...
\paragraph{WFLO Setting and Measurements}
...
We have measured solutions using the SS energy expression.
To to this end, we converted solutions that came from LS based methods into the corresponding SS expression.
Lastly, the total mean energy is presented in $kW/year$. 
...
We used state of the art software, namely Gurobi~\cite{gurobi}, to solve QCOP, ILP-SS and QP-SS, and 
applied both Fujitsu's digital annealer and GUrobi to solve QUBO.


\paragraph{Experimental Procedure}

\todo{Explain how we solved WFLO using the various instances}
...
\begin{table}[ht]
	...
	\vspace{0.5em}
	\caption{Total Expected Power (kW/year) after 100 seconds as function of benchmark and method \todo{Change to 100 seconds and re-arrange our benchmarks first.} }\label{tab:results1}
\end{table}
...
\vspace{0.5em}
\caption{Total Expected Power (kW/year) after 3600 seconds as function of benchmark and method}\label{tab:results2}
\end{table}
