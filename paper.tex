...
to model the energy objective as a quadratic function as, indeed, has been done in some existing optimization approaches. However, state-of-the-art solutions often trade-off between efficiency (providing quick designs) and accuracy (guaranteeing optimal energy solutions). In this work, we aim to find a balancing approach that would enable to obtain WFLO solutions quickly for interactive design \emph{and} solve the problem to optimality when accuracy is the goal. To this end, we exploit recent advances in both optimization hardware and software that target quadratic constraints: commercial mixed integer linear solvers have been extended to address some quadratic problems and nascent specialized hardware, including quantum computing systems, have focused on solving quadratic unconstrained binary optimization (QUBO) problems. 
%It is a key problem 
...
that
improve the state of the art and strike a balance between efficiency and optimality. In particular, the QUBO model delivers high quality solutions very quickly while the QCOP model can be used to find better solutions and provide quality guarantees over a longer run-time.
\end{abstract}
\begin{keyword}
