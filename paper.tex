...
of the procedure was recorded. \todo{@Chris: do we want to share the code that ran the experiments in batches? perhaps it should be separated from the problem construction and the two need to be shared separately.} \todo{JCB: As I requested last fall, we should have separate problem instance files from the code and make at least the problem instances available.} 
...
First, the two problems are the basis for the QP-LS and QUBO-LS methods that solved with Gurobi. This turns out
to be a powerful method as it gives the best performance after 3600 seconds for most instances (7 out of 12),
and especially,
for the hard instances with 36 wind directions. \todo{Avoid ``this'' as a pronoun. The previous sentence is a good example. I do not know what ``this'' refers to, especially as two models are referred to. Also how do you get 7/12?} In addition, the runtime performance of QP-LS is second
...
instances, shows competitive performance on the other instances, and outperforms all methods in terms of runtime by two orders of magnitude.\todo{JCB: Is ``two orders of magnitude'' accurate? If you compare to 100, it is only 1 order of magnitude to achieve the ``same'' performance.}
